...
class VTodoCompleter {
  /**
   * Create a VTodoCompleter. Reads and writes go through lib/webDavHelper,
   * which keeps its own logged-in client per account.
   ...
   * @param {AbortSignal} [options.signal] - Once aborted (the toggle timed out),
   *   no further write starts; a write already sent cannot be taken back.
   */
  constructor(options = {}) {
  ...
  async uncompleteVTodo(config, filename) {
    ...
    await this.write(config, filename, this.generateICS(parsed));
    return { original: filename };
  }
  ...
  async handleRecurrence(config, parsed, filename, completedDate) {
    ...
    const newFilename = this.buildOccurrenceFilename(filename, originalUID, newUID);
     ...
     */
    const completedOccurrence = this.buildCompletedOccurrence(parsed, completedDate, newUID);
     ...
     */
    await this.write(config, newFilename, this.generateICS(completedOccurrence), { create: true });
  ...
  findNextOccurrence(parsed) {
    ...
    const after = currentDue.getTime() > todayStart.getTime() ? currentDue : todayStart;
  ...
  markCompleted(parsed, completedDate) {
    this.setProperty(parsed, "VTODO", "COMPLETED", formatIcsDate(completedDate), "CREATED");
  ...
  consumeRuleCount(parsed, nextDate) {
    ...
    const remaining = rule.all().filter((date) => date.getTime() >= nextDate.getTime()).length;
    ...
    this.setProperty(parsed, "VTODO", "RRULE", value.replace(/COUNT=\d+/i, `COUNT=${remaining}`));
  }

  /**
   * Move a series to its next occurrence and open it again.
   * @param {Array} parsed - The parsed ICS data.
   * @param {Date} maxDate - The next occurrence (the new DTSTART, or DUE without one).
   */
  updateVTodoItem(parsed, maxDate) {
    ...
    if (previousStart && previousDue) {
      const span = parseIcsDate(previousDue, "jsDate").getTime() - parseIcsDate(previousStart, "jsDate").getTime();
  ...
  moveAlarm(parsed, maxDate, previousDue) {
    ...
    if (previousDue) {
      const offset = parseIcsDate(trigger, "jsDate").getTime() - parseIcsDate(previousDue, "jsDate").getTime();
  ...
  /**
   * Write one object, unless the operation was aborted in the meantime. A
   * recurring task takes two writes; stopping between them after a timeout
   * keeps a retried toggle from moving the series twice.
   * @returns {Promise<Object>} The writer's result.
   */
  write(config, filename, data, options) {
    this.signal?.throwIfAborted();
    return this.putFileContents(config, filename, data, options);
  }
  ...
  parseICS(icsContent) {
    ...
    /*
     * RFC 5545 folds long content lines: a line starting with a space or tab
     * continues the previous one. Parse the unfolded line, but keep the raw
     * lines as `original`, so an untouched property is written back exactly as
     * it was read.
     */
    const logicalLines = [];
    for (const raw of rawLines) {
      const previous = logicalLines[logicalLines.length - 1];
      if (previous && /^[ \t]/.test(raw)) {
        previous.raw.push(raw);
        previous.line += raw.slice(1);
      } else {
        logicalLines.push({ raw: [raw], line: raw });
      }
    }

    return logicalLines.map(({ raw, line }) => {
      const [keyPart, ...valueParts] = line.split(":");
      const value = valueParts.join(":");
      const [key, ...params] = keyPart.split(";");
      this.trackComponent(line);
      return {
        original: raw.join("\r\n"),
        line,
        key,
  ...
  /**
   * Track the component nesting (componentStack, currentDepth) for one ICS line.
  ...
  getElementLine(parsed, component, key) {
    return parsed.find((i) => i.key === key && i.component === component && !i.delete)?.line || null;
  }
  ...
  getElementValue(parsed, component, key, original = false) {
    const element = parsed.find((i) => i.key === key && i.component === component && !i.delete);
    if (!element) {
      return null;
    }
    if (original && !element.add) {
      return element.line.split(":")[1] || null;
    }
  ...
  addProperty(parsed, component, key, value, params = "", addBefore = "END") {
    ...
    const endIndex =
      addBefore === "END"
        ...
        : parsed.findIndex((i) => i.key === addBefore && i.component === component);
  ...
  removeComponent(parsed, component) {
    for (const item of parsed) {
      ...
      if (item.component === component || (item.key === "END" && item.value === component)) {
        item.delete = true;
      }
    }
  }
  ...
  buildLine(item) {
    ...
    if (dateTimeFields.includes(item.key)) {
      ...
      const original = (item.line ?? item.original).split(":")[1];
