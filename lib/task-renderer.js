...
class TaskRenderer {
  ...
  /**
   * Earliest hideAt still ahead, i.e. when the display has to be redrawn next. One that already
   * passed is skipped: the task stays in the data until the next fetch, and redrawing for it
   * would schedule the same past moment again after every render.
   *
   * @param {Array|null} toDoList - Calendars with their tasks
   * @param {number} [nowMs] - Reference time in ms
   * @returns {number|null} Timestamp in ms, or null if nothing counts down
   */
  static nextHideAt(toDoList, nowMs = Date.now()) {
    let next = null;
    const walk = (tasks) => {
      for (const task of tasks || []) {
        if (task.hideAt > nowMs && (next === null || task.hideAt < next)) {
          next = task.hideAt;
        }
        walk(task.children);
      }
