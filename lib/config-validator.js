...
const CONFIG_SCHEMA = {
  ...
  updateInterval: { type: "number", default: 10 * 60 * 1000, min: 1000 },
  backgroundRefresh: { type: "boolean", default: true },
  quietHours: { type: "object", default: null },
  requestTimeout: { type: "number", default: 30000, min: 1000 },
  frontendTimeout: { type: "number", default: 60000, min: 1000 },
  sortMethod: {
    type: "string",
    default: "priority",
    enum: ["priority", "priority desc", "created", "created desc", "modified", "modified desc"],
  },
  ...
  completedTaskGracePeriod: { type: "number", default: 60, min: 0 },
  ...
  // Optional, no default: unset means MagicMirror's global logLevel alone.
  logLevel: { type: "string", enum: ["none", "error", "warn", "info", "debug"] },
};
  ...
  if (schema.schema && typeof value === "object" && !Array.isArray(value)) {
    for (const [nestedKey, nestedSchema] of Object.entries(schema.schema)) {
      ...
      if (nestedValue !== undefined) {
        const nestedErrors = validateValue(`${key}.${nestedKey}`, nestedValue, nestedSchema);
...
/**
 * Validate a frontend config and merge in the schema defaults. Runs once per
 * instance, at CONFIGURE.
...
function normalizeConfig(config) {
  ...
  if (!valid) {
    throw new Error(`Configuration error: ${errors.map((e) => e.message).join("; ")}`);
  }
