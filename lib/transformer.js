...
function transformData(children, parents = []) {
  ...
  for (const child of children) {
    if (typeof child["related-to"] === "undefined") {
      ...
      const parent = findParent(
        parents,
        typeof child["related-to"].val === "undefined" ? child["related-to"] : child["related-to"].val,
      );
      if (parent) {
  ...
  /*
   * Retry orphans only while a pass still resolves at least one of them.
   * Without that guard, a RELATED-TO pointing at a task that is missing
   * (deleted, or in a calendar that is not included) recurses forever.
   */
  if (orphans.length > 0) {
    if (orphans.length < children.length) {
      return transformData(orphans, parents);
    }
    parents.push(...orphans);
  }
