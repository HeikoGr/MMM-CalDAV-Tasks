...
- `lib/` (except `lib/mmm-shared/`)
- `node_helper.js`, `MMM-CalDAV-Tasks.js`, `MMM-CalDAV-Tasks.css`
...
`tasks`, `sync`, `webdav`, `render`, `sort`, `transform`, `config`, `cli`, `deps`, `devcontainer`
...
- Good: `fix(webdav): retry fetchList on socket hung up errors`
- Bad: `fix(webdav): update webDavHelper.js`
- Bad: `fix(webdav): various fixes and improvements`
...
```
feat(config)!: require webDavAuth as an object

BREAKING CHANGE: the top-level `listUrl`/`username`/`password` keys are gone. Move each CalDAV
account into the `webDavAuth` object.
```

## Examples from this repository

```
fix(webdav): retry webDavHelper fetchList when the server closes the connection early
feat(tasks): support long-press to toggle vtodo-completer completion on touch screens
refactor(render): move priority-icon markup into task-renderer.js
fix(cli): stop cli-debug.js mistaking a flag value for the positional config path
docs: correct CLI debug instructions in docs/CLI-DEBUG.md
